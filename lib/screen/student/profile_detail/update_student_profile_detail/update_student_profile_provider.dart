...
class UpdateStudentProfileProvider extends ChangeNotifier {
  ...
  Future<bool> getStudentProfileDetail(dynamic context, String id) async {
    ...
    try {
      ...
      if (response["responseStatus"] == true &&
        ...
        notifyListeners();
        showSnackBar(
          response["responseMessage"] ?? "No student data found",
          context,
        );
  ...
  Future<bool> getPsychoStudentSkillData(
    context,
    String studentId,
    String skillId,
  ) async {
    ...
    try {
      final response = await _api.getApiCall(
        url:
            "${ApiServiceUrl.hamaareSitaareApiBaseUrl}${ApiServiceUrl.studentSkill}",
        params: {
      ...
      if (response["responseStatus"] == true && response["data"] is List) {
        _psychoSkillData = (response["data"] as List)
            .map(
              (e) => UpdateStudentPsychoSkillDataModel.fromJson(
                Map<String, dynamic>.from(e),
              ),
            )
            .toList();
        ...
        showSnackBar(
          response["responseMessage"] ?? "Invalid data received",
          context,
        );
      }
    } catch (e) {
  ...
  Future<bool> updatePsychoStudentSkillData(
      String studentId,
      String skillId,
      String qualityText,
    ...
    try {
      ...
      final ApiResponse apiResp = ResponseChecker.fromRaw(response);
  ...
  void updateChildSkillRating(int id, int ratingId) {
    final skills = _psychoSkillData?.first.skillQuality ?? [];

    for (final skill in skills) {

      if ((skill.skillQualityParent == null ||
          skill.skillQualityParent!.isEmpty) &&
          skill.qualityId == id) {
        skill.ratingId = ratingId;
        notifyListeners();
        return;
      }


      for (final child in skill.skillQualityParent ?? []) {
        if (child.qualityParentId == id) {
